...
export class CrearYoutubeComponent implements OnInit, OnDestroy {
	...
	guardarEntradaYoutube() {
		...
		this.formEntrada.get('creador').setValue(
			this.securityService.getDecodedAccessToken()
				.User);
		if (this.formEntrada.valid) {
			this.subscription$ = this.youtubeService.guardarEntradaYoutube(this.formEntrada.value)
				.subscribe((data) => {
